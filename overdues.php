...
<html>
<head>
      <title>view insurance status</title>
...
$q = " SELECT * FROM clients  JOIN insurance on clients.clientNo=insurance.clientNo";
...
if($num >= 1){	
	...
	echo "<button type='text'  class='btn btn-primary' data-toggle='collapse'  style='margin-bottom:10px;margin-top:5px;height:40px;width:150px'>{$status}</button><br>";
	echo "<a  href='overdues.php?client=$ClientNo' type='text'  style='margin-bottom:10px;margin-top:5px;height:40px;width:150px'>BLOCK/UNBLOCK</a>";
	echo "</li>";
...
if (isset($_GET['client'])) {
     myfun($_GET['client'],$con);
  }

function myfun($clientno,$con){
	$q = " SELECT * FROM insurance where clientNo='$clientno'";
    $result = mysqli_query($con, $q);
    $num = mysqli_num_rows($result);
    if($num >= 1){	
	...
	$status=$arr['status'];
	if($status=="active"){
		$q2= "Update insurance SET status='blocked' where clientNo=$clientno";
		mysqli_query($con, $q2);
...
echo "<script type='text/javascript'>document.location.href='{$URL}';</script>";
	    ...
	    $q2= "Update insurance SET status='active' where clientNo=$clientno";
	    mysqli_query($con, $q2);
...
<?php
 ...
 $query = " SELECT * FROM clients JOIN insurance on clients.clientNo=insurance.clientNo";
  $result = mysqli_query($con, $query);
  $num = mysqli_num_rows($result);
  if($num >= 1){
	  echo "<table><tbody>";
	  echo "<thead> <th>clintNo</th><th>Firstname</th><th>lastnames</th><th>gender/th><th>nationality</th>
	  <th>occupation</th><th>Date of birth</th><th>contact</th><th>insrance type</th><th>nationalid</th><th>Insurance Status</th>
	  </thead>";
	  while($arr=$result->fetch_assoc()){
		  echo "<tr>";
